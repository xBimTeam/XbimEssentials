		...
		public bool ValidateClause(Where.IfcPhysicalComplexQuantity clause) {
			...
			if (clause == Where.IfcPhysicalComplexQuantity.WR21) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc2x3.QuantityResource.IfcPhysicalComplexQuantity");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcPhysicalComplexQuantity.WR21' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
