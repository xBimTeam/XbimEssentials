...
namespace Xbim.Ifc2x3.QuantityResource
{
	...
	{
		private static readonly ILog Log = LogManager.GetLogger("Xbim.Ifc2x3.QuantityResource.IfcPhysicalComplexQuantity");
		...
		public bool ValidateClause(Where.IfcPhysicalComplexQuantity clause) {
			var retVal = false;
			if (clause == Where.IfcPhysicalComplexQuantity.WR21) {
				try {
					retVal = SIZEOF(HasQuantities.Where(temp => Object.ReferenceEquals(this, temp))) == 0;
				} catch (Exception ex) {
					Log.Error($"Exception thrown evaluating where-clause 'IfcPhysicalComplexQuantity.WR21' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
		...
		{
			if (!ValidateClause(Where.IfcPhysicalComplexQuantity.WR21))
...
namespace Xbim.Ifc2x3.Where
{
	...
	{
		public static readonly IfcPhysicalComplexQuantity WR21 = new IfcPhysicalComplexQuantity();
