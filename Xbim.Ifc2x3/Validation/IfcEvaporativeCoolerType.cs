...
namespace Xbim.Ifc2x3.HVACDomain
{
	...
	{
		private static readonly ILog Log = LogManager.GetLogger("Xbim.Ifc2x3.HVACDomain.IfcEvaporativeCoolerType");
		...
		public bool ValidateClause(Where.IfcEvaporativeCoolerType clause) {
			...
			if (clause == Where.IfcEvaporativeCoolerType.WR1) {
				try {
					...
					Log.Error($"Exception thrown evaluating where-clause 'IfcEvaporativeCoolerType.WR1' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
