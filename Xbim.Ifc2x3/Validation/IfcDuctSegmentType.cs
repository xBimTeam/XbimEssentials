		...
		public bool ValidateClause(Where.IfcDuctSegmentType clause) {
			...
			if (clause == Where.IfcDuctSegmentType.WR1) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc2x3.HVACDomain.IfcDuctSegmentType");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcDuctSegmentType.WR1' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
