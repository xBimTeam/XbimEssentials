...
namespace Xbim.Ifc2x3.GeometryResource
{
	...
	{
		private static readonly ILog Log = LogManager.GetLogger("Xbim.Ifc2x3.GeometryResource.IfcCartesianTransformationOperator2DnonUniform");
		...
		public bool ValidateClause(Where.IfcCartesianTransformationOperator2DnonUniform clause) {
			...
			if (clause == Where.IfcCartesianTransformationOperator2DnonUniform.WR1) {
				try {
					...
					Log.Error($"Exception thrown evaluating where-clause 'IfcCartesianTransformationOperator2DnonUniform.WR1' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
