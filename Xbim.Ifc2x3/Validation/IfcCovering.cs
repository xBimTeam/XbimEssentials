		...
		public bool ValidateClause(Where.IfcCovering clause) {
			...
			if (clause == Where.IfcCovering.WR61) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc2x3.ProductExtension.IfcCovering");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcCovering.WR61' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
