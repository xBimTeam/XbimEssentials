		...
		public bool ValidateClause(Where.IfcExternalReference clause) {
			...
			if (clause == Where.IfcExternalReference.WR1) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc2x3.ExternalReferenceResource.IfcExternalReference");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcExternalReference.WR1' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
