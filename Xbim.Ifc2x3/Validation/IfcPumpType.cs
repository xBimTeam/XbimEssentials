...
namespace Xbim.Ifc2x3.HVACDomain
{
	...
	{
		private static readonly ILog Log = LogManager.GetLogger("Xbim.Ifc2x3.HVACDomain.IfcPumpType");
		...
		public bool ValidateClause(Where.IfcPumpType clause) {
			...
			if (clause == Where.IfcPumpType.WR1) {
				try {
					...
					Log.Error($"Exception thrown evaluating where-clause 'IfcPumpType.WR1' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
