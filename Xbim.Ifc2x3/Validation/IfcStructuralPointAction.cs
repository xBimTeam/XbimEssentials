		...
		public bool ValidateClause(Where.IfcStructuralPointAction clause) {
			...
			if (clause == Where.IfcStructuralPointAction.WR61) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc2x3.StructuralAnalysisDomain.IfcStructuralPointAction");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcStructuralPointAction.WR61' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
