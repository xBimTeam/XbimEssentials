		...
		public bool ValidateClause(Where.IfcGasTerminalType clause) {
			...
			if (clause == Where.IfcGasTerminalType.WR1) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc2x3.HVACDomain.IfcGasTerminalType");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcGasTerminalType.WR1' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
