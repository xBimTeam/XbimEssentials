		...
		public bool ValidateClause(Where.IfcDuctSilencerType clause) {
			...
			if (clause == Where.IfcDuctSilencerType.WR1) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc2x3.HVACDomain.IfcDuctSilencerType");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcDuctSilencerType.WR1' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
