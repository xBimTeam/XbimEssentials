...
namespace Xbim.Ifc2x3.GeometryResource
{
	...
	{
		private static readonly ILog Log = LogManager.GetLogger("Xbim.Ifc2x3.GeometryResource.IfcAxis1Placement");
		...
		public bool ValidateClause(Where.IfcAxis1Placement clause) {
			...
			if (clause == Where.IfcAxis1Placement.WR1) {
				try {
					...
					Log.Error($"Exception thrown evaluating where-clause 'IfcAxis1Placement.WR1' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
			if (clause == Where.IfcAxis1Placement.WR2) {
				try {
					...
					Log.Error($"Exception thrown evaluating where-clause 'IfcAxis1Placement.WR2' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
