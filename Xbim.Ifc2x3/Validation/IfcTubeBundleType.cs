		...
		public bool ValidateClause(Where.IfcTubeBundleType clause) {
			...
			if (clause == Where.IfcTubeBundleType.WR1) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc2x3.HVACDomain.IfcTubeBundleType");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcTubeBundleType.WR1' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
