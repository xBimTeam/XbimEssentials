		...
		public bool ValidateClause(Where.IfcZShapeProfileDef clause) {
			...
			if (clause == Where.IfcZShapeProfileDef.WR21) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc2x3.ProfileResource.IfcZShapeProfileDef");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcZShapeProfileDef.WR21' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
