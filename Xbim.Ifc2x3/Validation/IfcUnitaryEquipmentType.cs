		...
		public bool ValidateClause(Where.IfcUnitaryEquipmentType clause) {
			...
			if (clause == Where.IfcUnitaryEquipmentType.WR1) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc2x3.HVACDomain.IfcUnitaryEquipmentType");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcUnitaryEquipmentType.WR1' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
