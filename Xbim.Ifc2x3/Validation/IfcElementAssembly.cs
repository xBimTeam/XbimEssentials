		...
		public bool ValidateClause(Where.IfcElementAssembly clause) {
			...
			if (clause == Where.IfcElementAssembly.WR1) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc2x3.ProductExtension.IfcElementAssembly");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcElementAssembly.WR1' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
