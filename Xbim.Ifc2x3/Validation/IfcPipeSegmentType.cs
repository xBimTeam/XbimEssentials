		...
		public bool ValidateClause(Where.IfcPipeSegmentType clause) {
			...
			if (clause == Where.IfcPipeSegmentType.WR1) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc2x3.HVACDomain.IfcPipeSegmentType");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcPipeSegmentType.WR1' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
