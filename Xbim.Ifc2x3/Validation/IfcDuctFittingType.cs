		...
		public bool ValidateClause(Where.IfcDuctFittingType clause) {
			...
			if (clause == Where.IfcDuctFittingType.WR2) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc2x3.HVACDomain.IfcDuctFittingType");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcDuctFittingType.WR2' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
