		...
		public bool ValidateClause(Where.IfcSpaceHeaterType clause) {
			...
			if (clause == Where.IfcSpaceHeaterType.WR1) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc2x3.HVACDomain.IfcSpaceHeaterType");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcSpaceHeaterType.WR1' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
