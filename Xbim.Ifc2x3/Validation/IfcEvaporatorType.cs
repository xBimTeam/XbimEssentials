		...
		public bool ValidateClause(Where.IfcEvaporatorType clause) {
			...
			if (clause == Where.IfcEvaporatorType.WR1) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc2x3.HVACDomain.IfcEvaporatorType");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcEvaporatorType.WR1' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
