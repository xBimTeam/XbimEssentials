		...
		public bool ValidateClause(Where.IfcMechanicalMaterialProperties clause) {
			...
			if (clause == Where.IfcMechanicalMaterialProperties.WR21) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc2x3.MaterialPropertyResource.IfcMechanicalMaterialProperties");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcMechanicalMaterialProperties.WR21' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
			if (clause == Where.IfcMechanicalMaterialProperties.WR22) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc2x3.MaterialPropertyResource.IfcMechanicalMaterialProperties");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcMechanicalMaterialProperties.WR22' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
