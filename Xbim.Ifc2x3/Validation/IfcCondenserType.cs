		...
		public bool ValidateClause(Where.IfcCondenserType clause) {
			...
			if (clause == Where.IfcCondenserType.WR1) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc2x3.HVACDomain.IfcCondenserType");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcCondenserType.WR1' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
