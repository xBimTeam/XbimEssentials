...
using Xbim.IfcRail.IfcFunctions;
...
namespace Xbim.IfcRail
{
    public static class Functions
    {
        ...
        internal static IEnumerable<IPersistEntity> USEDIN(IPersistEntity ifcObject, string v)
        {
            ...
            switch (v)
            {
                case "IFCRELASSOCIATES.RELATEDOBJECTS":
        ...
        internal static bool IfcShapeRepresentationTypes(IfcLabel? RepType, IItemSet<IfcRepresentationItem> Items)
        {
            ...
            switch (RepType.Value)
            {
                case "Point":
                    ...
                    break;
                case "PointCloud":
                    Count = Items.Count(x => x is IfcCartesianPointList3D);
                    break;
                case "Curve":
                ...
                case "AdvancedSurface":
                    Count = Items.Count(x => x is IfcBSplineSurface);
                    break;
                ...
                case "GeometricCurveSet":
                    ...
                    foreach (var ifcRepresentationItem in Items)
                    {
                        ...
                        {
                            var asIfcGeometricSet = ifcRepresentationItem as IfcGeometricSet;
                            if (asIfcGeometricSet.Elements.Count(temp => temp is IfcSurface) > 0)
                            {
                                Count--;
                            }
                        }
                ...
                case "Tessellation":
                    Count = Items.Count(x => x is IfcTessellatedItem);
                    break;
                case "SurfaceOrSolidModel":
                    Count = Items.Count(x =>
                            x is IfcTessellatedItem
                ...
                case "SweptSolid":
                    ...
                    Count = Items.Count(x =>
                            ...
                            && !(
                                x is IfcExtrudedAreaSolidTapered
                                || x is IfcRevolvedAreaSolidTapered)
                    );
                    break;
        ...
        internal static bool? IfcCorrectObjectAssignment(IfcObjectTypeEnum Constraint, IEnumerable<IfcObjectDefinition> Objects)
        {
            ...
            switch (Constraint)
            {
                ...
                case IfcObjectTypeEnum.PRODUCT:
                    {
                        Count = SIZEOF(Objects.Where(temp => !((INTYPEOF(temp, "IFCPRODUCT")))));
                        return (Count == 0);
                    }
                case IfcObjectTypeEnum.PROCESS:
                    {
                        Count = SIZEOF(Objects.Where(temp => !((INTYPEOF(temp, "IFCPROCESS")))));
                        return (Count == 0);
                    }
                    ;
                case IfcObjectTypeEnum.CONTROL:
                    {
                        Count = SIZEOF(Objects.Where(temp => !((INTYPEOF(temp, "IFCCONTROL")))));
                        return (Count == 0);
                    }
                    ;
                case IfcObjectTypeEnum.RESOURCE:
                    {
                        Count = SIZEOF(Objects.Where(temp => !((INTYPEOF(temp, "IFCRESOURCE")))));
                        return (Count == 0);
                    }
                    ;
                case IfcObjectTypeEnum.ACTOR:
                    {
                        Count = SIZEOF(Objects.Where(temp => !((INTYPEOF(temp, "IFCACTOR")))));
                        return (Count == 0);
                    }
                    ;
                case IfcObjectTypeEnum.GROUP:
                    {
                        Count = SIZEOF(Objects.Where(temp => !((INTYPEOF(temp, "IFCGROUP")))));
                        return (Count == 0);
                    }
                    ;
                case IfcObjectTypeEnum.PROJECT:
                    {
                        Count = SIZEOF(Objects.Where(temp => !((INTYPEOF(temp, "IFCPROJECT")))));
                        return (Count == 0);
                    }
                    ;
                default:
        ...
        internal static bool IfcCorrectFillAreaStyle(IItemSet<IfcFillStyleSelect> Styles)
        {
            ...
            int Colour = 0;
            int External = 0;
            External = SIZEOF(Styles.Where(Style => INTYPEOF(Style, "IFCEXTERNALLYDEFINEDHATCHSTYLE")));
            Hatching = SIZEOF(Styles.Where(Style => INTYPEOF(Style, "IFCFILLAREASTYLEHATCHING")));
            Tiles = SIZEOF(Styles.Where(Style => INTYPEOF(Style, "IFCFILLAREASTYLETILES")));
            Colour = SIZEOF(Styles.Where(Style => INTYPEOF(Style, "IFCCOLOUR")));
            if ((External > 1))
            {
                return (false);
            }
        ...
        internal static bool IfcSurfaceWeightsPositive(IfcRationalBSplineSurfaceWithKnots B)
        {
            ...
            // todo: check indices range from specs... possible bug in express
            for (var i = 1; i <= (B as IfcBSplineSurface).UUpper; i++)
            {
                for (var j = 1; j <= (B as IfcBSplineSurface).VUpper; j++)
                {
