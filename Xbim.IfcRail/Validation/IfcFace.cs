			...
			try
			{
				switch (clause)
				{
					case IfcFaceClause.HasOuterBound:
						retVal = Functions.SIZEOF(Bounds.Where(temp => Functions.TYPEOF(temp).Contains("IFCFACEOUTERBOUND"))) <= 1;
						break;
				}
			} catch (Exception  ex) {
				var log = Validation.ValidationLogging.CreateLogger<Xbim.IfcRail.TopologyResource.IfcFace>();
