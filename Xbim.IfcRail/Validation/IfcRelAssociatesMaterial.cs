			...
			try
			{
				switch (clause)
				{
					case IfcRelAssociatesMaterialClause.NoVoidElement:
						retVal = Functions.SIZEOF(this/* as IfcRelAssociates*/.RelatedObjects.Where(temp => (Functions.TYPEOF(temp).Contains("IFCFEATUREELEMENTSUBTRACTION")) || (Functions.TYPEOF(temp).Contains("IFCVIRTUALELEMENT")))) == 0;
						break;
					case IfcRelAssociatesMaterialClause.AllowedElements:
						retVal = Functions.SIZEOF(this/* as IfcRelAssociates*/.RelatedObjects.Where(temp => (Functions.SIZEOF(Functions.TYPEOF(temp) * Functions.NewTypesArray("IFCELEMENT", "IFCELEMENTTYPE", "IFCWINDOWSTYLE", "IFCDOORSTYLE", "IFCSTRUCTURALMEMBER", "IFCPORT")) == 0))) == 0;
						break;
				}
			} catch (Exception  ex) {
				var log = Validation.ValidationLogging.CreateLogger<Xbim.IfcRail.ProductExtension.IfcRelAssociatesMaterial>();
