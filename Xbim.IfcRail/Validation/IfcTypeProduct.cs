			...
			try
			{
				switch (clause)
				{
					case IfcTypeProductClause.ApplicableOccurrence:
						retVal = !(Functions.EXISTS(this/* as IfcTypeObject*/.Types.ItemAt(0))) || (Functions.SIZEOF(this/* as IfcTypeObject*/.Types.ItemAt(0).RelatedObjects.Where(temp => !(Functions.TYPEOF(temp).Contains("IFCPRODUCT")))) == 0);
						break;
				}
			} catch (Exception  ex) {
				var log = Validation.ValidationLogging.CreateLogger<Xbim.IfcRail.Kernel.IfcTypeProduct>();
