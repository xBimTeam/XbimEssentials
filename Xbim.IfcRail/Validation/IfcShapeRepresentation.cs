			...
			try
			{
				switch (clause)
				{
					case IfcShapeRepresentationClause.CorrectContext:
						retVal = Functions.TYPEOF(this/* as IfcRepresentation*/.ContextOfItems).Contains("IFCGEOMETRICREPRESENTATIONCONTEXT");
						break;
					case IfcShapeRepresentationClause.NoTopologicalItem:
						retVal = Functions.SIZEOF(Items.Where(temp => (Functions.TYPEOF(temp).Contains("IFCTOPOLOGICALREPRESENTATIONITEM")) && (!(Functions.SIZEOF(Functions.NewTypesArray("IFCVERTEXPOINT", "IFCEDGECURVE", "IFCFACESURFACE") * Functions.TYPEOF(temp)) == 1)))) == 0;
						break;
				...
				var log = Validation.ValidationLogging.CreateLogger<Xbim.IfcRail.RepresentationResource.IfcShapeRepresentation>();
