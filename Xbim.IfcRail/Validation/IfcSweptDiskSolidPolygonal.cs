			...
			try
			{
				switch (clause)
				{
					...
					case IfcSweptDiskSolidPolygonalClause.DirectrixIsPolyline:
						retVal = Functions.TYPEOF(this/* as IfcSweptDiskSolid*/.Directrix).Contains("IFCPOLYLINE");
						break;
				}
			} catch (Exception  ex) {
				var log = Validation.ValidationLogging.CreateLogger<Xbim.IfcRail.GeometricModelResource.IfcSweptDiskSolidPolygonal>();
