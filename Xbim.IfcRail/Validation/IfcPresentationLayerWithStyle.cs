			...
			try
			{
				switch (clause)
				{
					case IfcPresentationLayerWithStyleClause.ApplicableOnlyToItems:
						retVal = Functions.SIZEOF(AssignedItems.Where(temp => (Functions.SIZEOF(Functions.TYPEOF(temp) * Functions.NewTypesArray("IFCGEOMETRICREPRESENTATIONITEM", "IFCMAPPEDITEM")) == 1))) == Functions.SIZEOF(AssignedItems);
						break;
				}
			} catch (Exception  ex) {
				var log = Validation.ValidationLogging.CreateLogger<Xbim.IfcRail.PresentationOrganizationResource.IfcPresentationLayerWithStyle>();
