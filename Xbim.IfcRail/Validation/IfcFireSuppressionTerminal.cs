			...
			try
			{
				switch (clause)
				{
					...
					case IfcFireSuppressionTerminalClause.CorrectTypeAssigned:
						retVal = (Functions.SIZEOF(IsTypedBy) == 0) || (Functions.TYPEOF(this/* as IfcObject*/.IsTypedBy.ItemAt(0).RelatingType).Contains("IFCFIRESUPPRESSIONTERMINALTYPE"));
						break;
				}
			} catch (Exception  ex) {
				var log = Validation.ValidationLogging.CreateLogger<Xbim.IfcRail.PlumbingFireProtectionDomain.IfcFireSuppressionTerminal>();
