			...
			try
			{
				switch (clause)
				{
					case IfcBuildingElementClause.MaxOneMaterialAssociation:
						retVal = Functions.SIZEOF(this/* as IfcObjectDefinition*/.HasAssociations.Where(temp => Functions.TYPEOF(temp).Contains("IFCRELASSOCIATESMATERIAL"))) <= 1;
						break;
				}
			} catch (Exception  ex) {
				var log = Validation.ValidationLogging.CreateLogger<Xbim.IfcRail.ProductExtension.IfcBuildingElement>();
