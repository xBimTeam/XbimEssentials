			...
			try
			{
				switch (clause)
				{
					case IfcSurfaceStyleClause.MaxOneShading:
						retVal = Functions.SIZEOF(this.Styles.Where(Style => Functions.TYPEOF(Style).Contains("IFCSURFACESTYLESHADING"))) <= 1;
						break;
					case IfcSurfaceStyleClause.MaxOneLighting:
						retVal = Functions.SIZEOF(this.Styles.Where(Style => Functions.TYPEOF(Style).Contains("IFCSURFACESTYLELIGHTING"))) <= 1;
						break;
					case IfcSurfaceStyleClause.MaxOneRefraction:
						retVal = Functions.SIZEOF(this.Styles.Where(Style => Functions.TYPEOF(Style).Contains("IFCSURFACESTYLEREFRACTION"))) <= 1;
						break;
					case IfcSurfaceStyleClause.MaxOneTextures:
						retVal = Functions.SIZEOF(this.Styles.Where(Style => Functions.TYPEOF(Style).Contains("IFCSURFACESTYLEWITHTEXTURES"))) <= 1;
						break;
					case IfcSurfaceStyleClause.MaxOneExtDefined:
						retVal = Functions.SIZEOF(this.Styles.Where(Style => Functions.TYPEOF(Style).Contains("IFCEXTERNALLYDEFINEDSURFACESTYLE"))) <= 1;
						break;
				}
			} catch (Exception  ex) {
				var log = Validation.ValidationLogging.CreateLogger<Xbim.IfcRail.PresentationAppearanceResource.IfcSurfaceStyle>();
