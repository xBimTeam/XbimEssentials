			...
			try
			{
				switch (clause)
				{
					case IfcAdvancedFaceClause.ApplicableSurface:
						retVal = Functions.SIZEOF(Functions.NewTypesArray("IFCELEMENTARYSURFACE", "IFCSWEPTSURFACE", "IFCBSPLINESURFACE") * Functions.TYPEOF(this/* as IfcFaceSurface*/.FaceSurface)) == 1;
						break;
					case IfcAdvancedFaceClause.RequiresEdgeCurve:
						retVal = Functions.SIZEOF(this/* as IfcFace*/.Bounds.Where(Bnds => Functions.TYPEOF(Bnds.Bound).Contains("IFCEDGELOOP")).Where(ElpFbnds => !(Functions.SIZEOF(ElpFbnds.Bound.AsIfcEdgeLoop().EdgeList.Where(Oe => !(Functions.TYPEOF(Oe/* as IfcOrientedEdge*/.EdgeElement).Contains("IFCEDGECURVE")))) == 0))) == 0;
						break;
					case IfcAdvancedFaceClause.ApplicableEdgeCurves:
						retVal = Functions.SIZEOF(this/* as IfcFace*/.Bounds.Where(Bnds => Functions.TYPEOF(Bnds.Bound).Contains("IFCEDGELOOP")).Where(ElpFbnds => !(Functions.SIZEOF(ElpFbnds.Bound.AsIfcEdgeLoop().EdgeList.Where(Oe => !(Functions.SIZEOF(Functions.NewTypesArray("IFCLINE", "IFCCONIC", "IFCPOLYLINE", "IFCBSPLINECURVE") * Functions.TYPEOF(Oe/* as IfcOrientedEdge*/.EdgeElement.AsIfcEdgeCurve().EdgeGeometry)) == 1))) == 0))) == 0;
						break;
				}
			} catch (Exception  ex) {
				var log = Validation.ValidationLogging.CreateLogger<Xbim.IfcRail.TopologyResource.IfcAdvancedFace>();
