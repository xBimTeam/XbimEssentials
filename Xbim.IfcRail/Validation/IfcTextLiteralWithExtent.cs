			...
			try
			{
				switch (clause)
				{
					case IfcTextLiteralWithExtentClause.WR31:
						retVal = !(Functions.TYPEOF(Extent).Contains("IFCPLANARBOX"));
						break;
				}
			} catch (Exception  ex) {
				var log = Validation.ValidationLogging.CreateLogger<Xbim.IfcRail.PresentationDefinitionResource.IfcTextLiteralWithExtent>();
