			...
			try
			{
				switch (clause)
				{
					case IfcStyledRepresentationClause.OnlyStyledItems:
						retVal = Functions.SIZEOF(this/* as IfcRepresentation*/.Items.Where(temp => (!(Functions.TYPEOF(temp).Contains("IFCSTYLEDITEM"))))) == 0;
						break;
				}
			} catch (Exception  ex) {
				var log = Validation.ValidationLogging.CreateLogger<Xbim.IfcRail.RepresentationResource.IfcStyledRepresentation>();
