			...
			try
			{
				switch (clause)
				{
					case IfcAdvancedBrepClause.HasAdvancedFaces:
						retVal = Functions.SIZEOF(this/* as IfcManifoldSolidBrep*/.Outer.CfsFaces.Where(Afs => (!(Functions.TYPEOF(Afs).Contains("IFCADVANCEDFACE"))))) == 0;
						break;
				}
			} catch (Exception  ex) {
				var log = Validation.ValidationLogging.CreateLogger<Xbim.IfcRail.GeometricModelResource.IfcAdvancedBrep>();
