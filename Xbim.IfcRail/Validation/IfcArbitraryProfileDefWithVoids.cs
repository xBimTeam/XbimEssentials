			...
			try
			{
				switch (clause)
				{
					...
					case IfcArbitraryProfileDefWithVoidsClause.WR3:
						retVal = Functions.SIZEOF(InnerCurves.Where(temp => Functions.TYPEOF(temp).Contains("IFCLINE"))) == 0;
						break;
				}
			} catch (Exception  ex) {
				var log = Validation.ValidationLogging.CreateLogger<Xbim.IfcRail.ProfileResource.IfcArbitraryProfileDefWithVoids>();
