			...
			try
			{
				switch (clause)
				{
					...
					case IfcPolygonalBoundedHalfSpaceClause.BoundaryType:
						retVal = Functions.SIZEOF(Functions.TYPEOF(PolygonalBoundary) * Functions.NewTypesArray("IFCPOLYLINE", "IFCCOMPOSITECURVE")) == 1;
						break;
				}
			} catch (Exception  ex) {
				var log = Validation.ValidationLogging.CreateLogger<Xbim.IfcRail.GeometricModelResource.IfcPolygonalBoundedHalfSpace>();
