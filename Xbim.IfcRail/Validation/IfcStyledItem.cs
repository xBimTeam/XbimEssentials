			...
			try
			{
				switch (clause)
				{
					case IfcStyledItemClause.ApplicableItem:
						retVal = !(Functions.TYPEOF(Item).Contains("IFCSTYLEDITEM"));
						break;
				}
			} catch (Exception  ex) {
				var log = Validation.ValidationLogging.CreateLogger<Xbim.IfcRail.PresentationAppearanceResource.IfcStyledItem>();
