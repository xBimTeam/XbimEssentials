			...
			try
			{
				switch (clause)
				{
					case IfcGeometricCurveSetClause.NoSurfaces:
						retVal = Functions.SIZEOF(this/* as IfcGeometricSet*/.Elements.Where(Temp => Functions.TYPEOF(Temp).Contains("IFCSURFACE"))) == 0;
						break;
				}
			} catch (Exception  ex) {
				var log = Validation.ValidationLogging.CreateLogger<Xbim.IfcRail.GeometricModelResource.IfcGeometricCurveSet>();
