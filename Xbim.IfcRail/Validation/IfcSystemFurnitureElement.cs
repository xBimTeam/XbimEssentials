			...
			try
			{
				switch (clause)
				{
					...
					case IfcSystemFurnitureElementClause.CorrectTypeAssigned:
						retVal = (Functions.SIZEOF(IsTypedBy) == 0) || (Functions.TYPEOF(this/* as IfcObject*/.IsTypedBy.ItemAt(0).RelatingType).Contains("IFCSYSTEMFURNITUREELEMENTTYPE"));
						break;
				}
			} catch (Exception  ex) {
				var log = Validation.ValidationLogging.CreateLogger<Xbim.IfcRail.SharedFacilitiesElements.IfcSystemFurnitureElement>();
