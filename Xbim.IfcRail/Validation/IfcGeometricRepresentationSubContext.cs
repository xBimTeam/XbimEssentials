			...
			try
			{
				switch (clause)
				{
					case IfcGeometricRepresentationSubContextClause.ParentNoSub:
						retVal = !(Functions.TYPEOF(ParentContext).Contains("IFCGEOMETRICREPRESENTATIONSUBCONTEXT"));
						break;
				...
				var log = Validation.ValidationLogging.CreateLogger<Xbim.IfcRail.RepresentationResource.IfcGeometricRepresentationSubContext>();
