			...
			try
			{
				switch (clause)
				{
					case IfcFillAreaStyleClause.MaxOneColour:
						retVal = Functions.SIZEOF(this.FillStyles.Where(Style => Functions.TYPEOF(Style).Contains("IFCCOLOUR"))) <= 1;
						break;
					case IfcFillAreaStyleClause.MaxOneExtHatchStyle:
						retVal = Functions.SIZEOF(this.FillStyles.Where(Style => Functions.TYPEOF(Style).Contains("IFCEXTERNALLYDEFINEDHATCHSTYLE"))) <= 1;
						break;
				...
				var log = Validation.ValidationLogging.CreateLogger<Xbim.IfcRail.PresentationAppearanceResource.IfcFillAreaStyle>();
