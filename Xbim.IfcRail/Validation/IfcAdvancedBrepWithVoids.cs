			...
			try
			{
				switch (clause)
				{
					case IfcAdvancedBrepWithVoidsClause.VoidsHaveAdvancedFaces:
						retVal = Functions.SIZEOF(Voids.Where(Vsh => Functions.SIZEOF(Vsh.CfsFaces.Where(Afs => (!(Functions.TYPEOF(Afs).Contains("IFCADVANCEDFACE"))))) == 0)) == 0;
						break;
				}
			} catch (Exception  ex) {
				var log = Validation.ValidationLogging.CreateLogger<Xbim.IfcRail.GeometricModelResource.IfcAdvancedBrepWithVoids>();
