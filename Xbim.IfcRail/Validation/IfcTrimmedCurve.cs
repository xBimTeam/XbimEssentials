			...
			try
			{
				switch (clause)
				{
					...
					case IfcTrimmedCurveClause.NoTrimOfBoundedCurves:
						retVal = !(Functions.TYPEOF(BasisCurve).Contains("IFCBOUNDEDCURVE"));
						break;
				}
			} catch (Exception  ex) {
				var log = Validation.ValidationLogging.CreateLogger<Xbim.IfcRail.GeometryResource.IfcTrimmedCurve>();
