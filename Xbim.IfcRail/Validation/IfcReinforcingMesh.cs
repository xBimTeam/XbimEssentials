			...
			try
			{
				switch (clause)
				{
					...
					case IfcReinforcingMeshClause.CorrectTypeAssigned:
						retVal = (Functions.SIZEOF(IsTypedBy) == 0) || (Functions.TYPEOF(this/* as IfcObject*/.IsTypedBy.ItemAt(0).RelatingType).Contains("IFCREINFORCINGMESHTYPE"));
						break;
				}
			} catch (Exception  ex) {
				var log = Validation.ValidationLogging.CreateLogger<Xbim.IfcRail.StructuralElementsDomain.IfcReinforcingMesh>();
