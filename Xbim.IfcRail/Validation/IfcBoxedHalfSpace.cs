			...
			try
			{
				switch (clause)
				{
					case IfcBoxedHalfSpaceClause.UnboundedSurface:
						retVal = !(Functions.TYPEOF(this/* as IfcHalfSpaceSolid*/.BaseSurface).Contains("IFCCURVEBOUNDEDPLANE"));
						break;
				}
			} catch (Exception  ex) {
				var log = Validation.ValidationLogging.CreateLogger<Xbim.IfcRail.GeometricModelResource.IfcBoxedHalfSpace>();
