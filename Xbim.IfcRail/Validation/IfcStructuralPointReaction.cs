			...
			try
			{
				switch (clause)
				{
					case IfcStructuralPointReactionClause.SuitableLoadType:
						retVal = Functions.SIZEOF(Functions.NewTypesArray("IFCSTRUCTURALLOADSINGLEFORCE", "IFCSTRUCTURALLOADSINGLEDISPLACEMENT") * Functions.TYPEOF(this/* as IfcStructuralActivity*/.AppliedLoad)) == 1;
						break;
				}
			} catch (Exception  ex) {
				var log = Validation.ValidationLogging.CreateLogger<Xbim.IfcRail.StructuralAnalysisDomain.IfcStructuralPointReaction>();
