			...
			try
			{
				switch (clause)
				{
					case IfcRepresentationMapClause.ApplicableMappedRepr:
						retVal = Functions.TYPEOF(MappedRepresentation).Contains("IFCSHAPEMODEL");
						break;
				}
			} catch (Exception  ex) {
				var log = Validation.ValidationLogging.CreateLogger<Xbim.IfcRail.GeometryResource.IfcRepresentationMap>();
