			...
			try
			{
				switch (clause)
				{
					case IfcProductClause.PlacementForShapeRepresentation:
						retVal = (Functions.EXISTS(Representation) && Functions.EXISTS(ObjectPlacement)) || (Functions.EXISTS(Representation) && (Functions.SIZEOF(Representation.Representations.Where(temp => Functions.TYPEOF(temp).Contains("IFCSHAPEREPRESENTATION"))) == 0)) || (!(Functions.EXISTS(Representation)));
						break;
				}
			} catch (Exception  ex) {
				var log = Validation.ValidationLogging.CreateLogger<Xbim.IfcRail.Kernel.IfcProduct>();
