			...
			try
			{
				switch (clause)
				{
					...
					case IfcArbitraryClosedProfileDefClause.WR2:
						retVal = !(Functions.TYPEOF(OuterCurve).Contains("IFCLINE"));
						break;
					case IfcArbitraryClosedProfileDefClause.WR3:
						retVal = !(Functions.TYPEOF(OuterCurve).Contains("IFCOFFSETCURVE2D"));
						break;
				}
			} catch (Exception  ex) {
				var log = Validation.ValidationLogging.CreateLogger<Xbim.IfcRail.ProfileResource.IfcArbitraryClosedProfileDef>();
