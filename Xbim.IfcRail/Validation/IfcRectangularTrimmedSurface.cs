			...
			try
			{
				switch (clause)
				{
					...
					case IfcRectangularTrimmedSurfaceClause.UsenseCompatible:
						retVal = ((Functions.TYPEOF(BasisSurface).Contains("IFCELEMENTARYSURFACE")) && (!(Functions.TYPEOF(BasisSurface).Contains("IFCPLANE")))) || (Functions.TYPEOF(BasisSurface).Contains("IFCSURFACEOFREVOLUTION")) || (Usense == (U2 > U1));
						break;
				...
				var log = Validation.ValidationLogging.CreateLogger<Xbim.IfcRail.GeometryResource.IfcRectangularTrimmedSurface>();
