...
namespace Xbim.Ifc4.HvacDomain
{
	...
	{
		private static readonly ILog Log = LogManager.GetLogger("Xbim.Ifc4.HvacDomain.IfcTankType");
		...
		public bool ValidateClause(Where.IfcTankType clause) {
			...
			if (clause == Where.IfcTankType.CorrectPredefinedType) {
				try {
					...
					Log.Error($"Exception thrown evaluating where-clause 'IfcTankType.CorrectPredefinedType' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
