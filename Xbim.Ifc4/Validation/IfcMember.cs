		...
		public bool ValidateClause(Where.IfcMember clause) {
			...
			if (clause == Where.IfcMember.CorrectPredefinedType) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.SharedBldgElements.IfcMember");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcMember.CorrectPredefinedType' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
			if (clause == Where.IfcMember.CorrectTypeAssigned) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.SharedBldgElements.IfcMember");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcMember.CorrectTypeAssigned' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
