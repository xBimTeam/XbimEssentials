...
namespace Xbim.Ifc4.SharedBldgElements
{
	...
	{
		private static readonly ILog Log = LogManager.GetLogger("Xbim.Ifc4.SharedBldgElements.IfcCoveringType");
		...
		public bool ValidateClause(Where.IfcCoveringType clause) {
			...
			if (clause == Where.IfcCoveringType.CorrectPredefinedType) {
				try {
					...
					Log.Error($"Exception thrown evaluating where-clause 'IfcCoveringType.CorrectPredefinedType' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
