...
namespace Xbim.Ifc4.ProductExtension
{
	...
	{
		private static readonly ILog Log = LogManager.GetLogger("Xbim.Ifc4.ProductExtension.IfcSpaceType");
		...
		public bool ValidateClause(Where.IfcSpaceType clause) {
			...
			if (clause == Where.IfcSpaceType.CorrectPredefinedType) {
				try {
					...
					Log.Error($"Exception thrown evaluating where-clause 'IfcSpaceType.CorrectPredefinedType' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
