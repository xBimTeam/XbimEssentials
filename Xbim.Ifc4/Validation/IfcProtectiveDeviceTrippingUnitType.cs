...
namespace Xbim.Ifc4.ElectricalDomain
{
	...
	{
		private static readonly ILog Log = LogManager.GetLogger("Xbim.Ifc4.ElectricalDomain.IfcProtectiveDeviceTrippingUnitType");
		...
		public bool ValidateClause(Where.IfcProtectiveDeviceTrippingUnitType clause) {
			...
			if (clause == Where.IfcProtectiveDeviceTrippingUnitType.CorrectPredefinedType) {
				try {
					...
					Log.Error($"Exception thrown evaluating where-clause 'IfcProtectiveDeviceTrippingUnitType.CorrectPredefinedType' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
