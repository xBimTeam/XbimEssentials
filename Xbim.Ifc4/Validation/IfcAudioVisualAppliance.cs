...
namespace Xbim.Ifc4.ElectricalDomain
{
	...
	{
		private static readonly ILog Log = LogManager.GetLogger("Xbim.Ifc4.ElectricalDomain.IfcAudioVisualAppliance");
		...
		public bool ValidateClause(Where.IfcAudioVisualAppliance clause) {
			...
			if (clause == Where.IfcAudioVisualAppliance.CorrectPredefinedType) {
				try {
					...
					Log.Error($"Exception thrown evaluating where-clause 'IfcAudioVisualAppliance.CorrectPredefinedType' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
			if (clause == Where.IfcAudioVisualAppliance.CorrectTypeAssigned) {
				try {
					retVal = (SIZEOF(IsTypedBy) == 0) || (TYPEOF(this/* as IfcObject*/.IsTypedBy.ToArray()[0].RelatingType).Contains("IFC4.IFCAUDIOVISUALAPPLIANCETYPE"));
				} catch (Exception ex) {
					Log.Error($"Exception thrown evaluating where-clause 'IfcAudioVisualAppliance.CorrectTypeAssigned' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
