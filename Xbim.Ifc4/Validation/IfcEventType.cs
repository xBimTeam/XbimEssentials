		...
		public bool ValidateClause(Where.IfcEventType clause) {
			...
			if (clause == Where.IfcEventType.CorrectPredefinedType) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.ProcessExtension.IfcEventType");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcEventType.CorrectPredefinedType' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
			if (clause == Where.IfcEventType.CorrectEventTriggerType) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.ProcessExtension.IfcEventType");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcEventType.CorrectEventTriggerType' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
