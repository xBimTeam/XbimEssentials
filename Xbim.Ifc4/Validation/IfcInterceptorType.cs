...
namespace Xbim.Ifc4.PlumbingFireProtectionDomain
{
	...
	{
		private static readonly ILog Log = LogManager.GetLogger("Xbim.Ifc4.PlumbingFireProtectionDomain.IfcInterceptorType");
		...
		public bool ValidateClause(Where.IfcInterceptorType clause) {
			...
			if (clause == Where.IfcInterceptorType.CorrectPredefinedType) {
				try {
					...
					Log.Error($"Exception thrown evaluating where-clause 'IfcInterceptorType.CorrectPredefinedType' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
