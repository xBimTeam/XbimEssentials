		...
		public bool ValidateClause(Where.IfcPile clause) {
			...
			if (clause == Where.IfcPile.CorrectPredefinedType) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.StructuralElementsDomain.IfcPile");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcPile.CorrectPredefinedType' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
			if (clause == Where.IfcPile.CorrectTypeAssigned) {
				try {
					retVal = (SIZEOF(IsTypedBy) == 0) || (TYPEOF(this/* as IfcObject*/.IsTypedBy.ItemAt(0).RelatingType).Contains("IFC4.IFCPILETYPE"));
				} catch (Exception ex) {
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.StructuralElementsDomain.IfcPile");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcPile.CorrectTypeAssigned' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
