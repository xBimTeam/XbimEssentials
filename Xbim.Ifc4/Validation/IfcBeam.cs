		...
		public bool ValidateClause(Where.IfcBeam clause) {
			...
			if (clause == Where.IfcBeam.CorrectPredefinedType) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.SharedBldgElements.IfcBeam");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcBeam.CorrectPredefinedType' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
			if (clause == Where.IfcBeam.CorrectTypeAssigned) {
				try {
					retVal = (SIZEOF(IsTypedBy) == 0) || (TYPEOF(this/* as IfcObject*/.IsTypedBy.ItemAt(0).RelatingType).Contains("IFC4.IFCBEAMTYPE"));
				} catch (Exception ex) {
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.SharedBldgElements.IfcBeam");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcBeam.CorrectTypeAssigned' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
