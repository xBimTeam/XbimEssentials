...
namespace Xbim.Ifc4.SharedComponentElements
{
	...
	{
		private static readonly ILog Log = LogManager.GetLogger("Xbim.Ifc4.SharedComponentElements.IfcFastener");
		...
		public bool ValidateClause(Where.IfcFastener clause) {
			...
			if (clause == Where.IfcFastener.CorrectPredefinedType) {
				try {
					...
					Log.Error($"Exception thrown evaluating where-clause 'IfcFastener.CorrectPredefinedType' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
			if (clause == Where.IfcFastener.CorrectTypeAssigned) {
				try {
					retVal = (SIZEOF(IsTypedBy) == 0) || (TYPEOF(this/* as IfcObject*/.IsTypedBy.ToArray()[0].RelatingType).Contains("IFC4.IFCFASTENERTYPE"));
				} catch (Exception ex) {
					Log.Error($"Exception thrown evaluating where-clause 'IfcFastener.CorrectTypeAssigned' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
