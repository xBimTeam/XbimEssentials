		...
		public bool ValidateClause(Where.IfcSystemFurnitureElementType clause) {
			...
			if (clause == Where.IfcSystemFurnitureElementType.CorrectPredefinedType) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.SharedFacilitiesElements.IfcSystemFurnitureElementType");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcSystemFurnitureElementType.CorrectPredefinedType' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
