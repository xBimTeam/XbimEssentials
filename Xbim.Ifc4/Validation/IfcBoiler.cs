		...
		public bool ValidateClause(Where.IfcBoiler clause) {
			...
			if (clause == Where.IfcBoiler.CorrectPredefinedType) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.HvacDomain.IfcBoiler");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcBoiler.CorrectPredefinedType' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
			if (clause == Where.IfcBoiler.CorrectTypeAssigned) {
				try {
					retVal = (SIZEOF(IsTypedBy) == 0) || (TYPEOF(this/* as IfcObject*/.IsTypedBy.ItemAt(0).RelatingType).Contains("IFC4.IFCBOILERTYPE"));
				} catch (Exception ex) {
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.HvacDomain.IfcBoiler");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcBoiler.CorrectTypeAssigned' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
