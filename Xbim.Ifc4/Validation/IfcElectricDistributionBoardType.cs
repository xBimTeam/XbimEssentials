		...
		public bool ValidateClause(Where.IfcElectricDistributionBoardType clause) {
			...
			if (clause == Where.IfcElectricDistributionBoardType.CorrectPredefinedType) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.ElectricalDomain.IfcElectricDistributionBoardType");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcElectricDistributionBoardType.CorrectPredefinedType' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
