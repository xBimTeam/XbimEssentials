		...
		public bool ValidateClause(Where.IfcReinforcingMesh clause) {
			...
			if (clause == Where.IfcReinforcingMesh.CorrectPredefinedType) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.StructuralElementsDomain.IfcReinforcingMesh");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcReinforcingMesh.CorrectPredefinedType' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
			if (clause == Where.IfcReinforcingMesh.CorrectTypeAssigned) {
				try {
					retVal = (SIZEOF(IsTypedBy) == 0) || (TYPEOF(this/* as IfcObject*/.IsTypedBy.ItemAt(0).RelatingType).Contains("IFC4.IFCREINFORCINGMESHTYPE"));
				} catch (Exception ex) {
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.StructuralElementsDomain.IfcReinforcingMesh");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcReinforcingMesh.CorrectTypeAssigned' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
