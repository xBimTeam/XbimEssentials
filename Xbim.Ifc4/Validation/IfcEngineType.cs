		...
		public bool ValidateClause(Where.IfcEngineType clause) {
			...
			if (clause == Where.IfcEngineType.CorrectPredefinedType) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.HvacDomain.IfcEngineType");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcEngineType.CorrectPredefinedType' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
