		...
		public bool ValidateClause(Where.IfcAirTerminalBox clause) {
			...
			if (clause == Where.IfcAirTerminalBox.CorrectPredefinedType) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.HvacDomain.IfcAirTerminalBox");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcAirTerminalBox.CorrectPredefinedType' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
			if (clause == Where.IfcAirTerminalBox.CorrectTypeAssigned) {
				try {
					retVal = (SIZEOF(IsTypedBy) == 0) || (TYPEOF(this/* as IfcObject*/.IsTypedBy.ItemAt(0).RelatingType).Contains("IFC4.IFCAIRTERMINALBOXTYPE"));
				} catch (Exception ex) {
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.HvacDomain.IfcAirTerminalBox");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcAirTerminalBox.CorrectTypeAssigned' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
