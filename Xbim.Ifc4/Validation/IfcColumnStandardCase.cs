		...
		public bool ValidateClause(Where.IfcColumnStandardCase clause) {
			...
			if (clause == Where.IfcColumnStandardCase.HasMaterialProfileSetUsage) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.SharedBldgElements.IfcColumnStandardCase");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcColumnStandardCase.HasMaterialProfileSetUsage' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
