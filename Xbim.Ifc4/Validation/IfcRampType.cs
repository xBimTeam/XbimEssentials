		...
		public bool ValidateClause(Where.IfcRampType clause) {
			...
			if (clause == Where.IfcRampType.CorrectPredefinedType) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.SharedBldgElements.IfcRampType");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcRampType.CorrectPredefinedType' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
