		...
		public bool ValidateClause(Where.IfcDamperType clause) {
			...
			if (clause == Where.IfcDamperType.CorrectPredefinedType) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.HvacDomain.IfcDamperType");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcDamperType.CorrectPredefinedType' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
