		...
		public bool ValidateClause(Where.IfcOutletType clause) {
			...
			if (clause == Where.IfcOutletType.CorrectPredefinedType) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.ElectricalDomain.IfcOutletType");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcOutletType.CorrectPredefinedType' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
