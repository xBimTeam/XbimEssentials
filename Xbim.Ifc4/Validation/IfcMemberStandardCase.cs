...
namespace Xbim.Ifc4.SharedBldgElements
{
	...
	{
		private static readonly ILog Log = LogManager.GetLogger("Xbim.Ifc4.SharedBldgElements.IfcMemberStandardCase");
		...
		public bool ValidateClause(Where.IfcMemberStandardCase clause) {
			...
			if (clause == Where.IfcMemberStandardCase.HasMaterialProfileSetUsage) {
				try {
					...
					Log.Error($"Exception thrown evaluating where-clause 'IfcMemberStandardCase.HasMaterialProfileSetUsage' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
