		...
		public bool ValidateClause(Where.IfcRoofType clause) {
			...
			if (clause == Where.IfcRoofType.CorrectPredefinedType) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.SharedBldgElements.IfcRoofType");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcRoofType.CorrectPredefinedType' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
