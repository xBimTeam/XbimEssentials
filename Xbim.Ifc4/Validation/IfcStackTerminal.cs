...
namespace Xbim.Ifc4.PlumbingFireProtectionDomain
{
	...
	{
		private static readonly ILog Log = LogManager.GetLogger("Xbim.Ifc4.PlumbingFireProtectionDomain.IfcStackTerminal");
		...
		public bool ValidateClause(Where.IfcStackTerminal clause) {
			...
			if (clause == Where.IfcStackTerminal.CorrectPredefinedType) {
				try {
					...
					Log.Error($"Exception thrown evaluating where-clause 'IfcStackTerminal.CorrectPredefinedType' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
			if (clause == Where.IfcStackTerminal.CorrectTypeAssigned) {
				try {
					retVal = (SIZEOF(IsTypedBy) == 0) || (TYPEOF(this/* as IfcObject*/.IsTypedBy.ToArray()[0].RelatingType).Contains("IFC4.IFCSTACKTERMINALTYPE"));
				} catch (Exception ex) {
					Log.Error($"Exception thrown evaluating where-clause 'IfcStackTerminal.CorrectTypeAssigned' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
