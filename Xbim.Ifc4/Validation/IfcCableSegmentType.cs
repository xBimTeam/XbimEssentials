...
namespace Xbim.Ifc4.ElectricalDomain
{
	...
	{
		private static readonly ILog Log = LogManager.GetLogger("Xbim.Ifc4.ElectricalDomain.IfcCableSegmentType");
		...
		public bool ValidateClause(Where.IfcCableSegmentType clause) {
			...
			if (clause == Where.IfcCableSegmentType.CorrectPredefinedType) {
				try {
					...
					Log.Error($"Exception thrown evaluating where-clause 'IfcCableSegmentType.CorrectPredefinedType' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
