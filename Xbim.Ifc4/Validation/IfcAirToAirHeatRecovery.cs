		...
		public bool ValidateClause(Where.IfcAirToAirHeatRecovery clause) {
			...
			if (clause == Where.IfcAirToAirHeatRecovery.CorrectPredefinedType) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.HvacDomain.IfcAirToAirHeatRecovery");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcAirToAirHeatRecovery.CorrectPredefinedType' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
			if (clause == Where.IfcAirToAirHeatRecovery.CorrectTypeAssigned) {
				try {
					retVal = (SIZEOF(IsTypedBy) == 0) || (TYPEOF(this/* as IfcObject*/.IsTypedBy.ItemAt(0).RelatingType).Contains("IFC4.IFCAIRTOAIRHEATRECOVERYTYPE"));
				} catch (Exception ex) {
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.HvacDomain.IfcAirToAirHeatRecovery");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcAirToAirHeatRecovery.CorrectTypeAssigned' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
