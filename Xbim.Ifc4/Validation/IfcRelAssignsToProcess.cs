...
namespace Xbim.Ifc4.Kernel
{
	...
	{
		private static readonly ILog Log = LogManager.GetLogger("Xbim.Ifc4.Kernel.IfcRelAssignsToProcess");
		...
		public bool ValidateClause(Where.IfcRelAssignsToProcess clause) {
			...
			if (clause == Where.IfcRelAssignsToProcess.NoSelfReference) {
				try {
					...
					Log.Error($"Exception thrown evaluating where-clause 'IfcRelAssignsToProcess.NoSelfReference' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
