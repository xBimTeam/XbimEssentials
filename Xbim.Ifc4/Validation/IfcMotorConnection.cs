		...
		public bool ValidateClause(Where.IfcMotorConnection clause) {
			...
			if (clause == Where.IfcMotorConnection.CorrectPredefinedType) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.ElectricalDomain.IfcMotorConnection");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcMotorConnection.CorrectPredefinedType' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
			if (clause == Where.IfcMotorConnection.CorrectTypeAssigned) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.ElectricalDomain.IfcMotorConnection");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcMotorConnection.CorrectTypeAssigned' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
