...
namespace Xbim.Ifc4.HvacDomain
{
	...
	{
		private static readonly ILog Log = LogManager.GetLogger("Xbim.Ifc4.HvacDomain.IfcAirTerminalBoxType");
		...
		public bool ValidateClause(Where.IfcAirTerminalBoxType clause) {
			...
			if (clause == Where.IfcAirTerminalBoxType.CorrectPredefinedType) {
				try {
					...
					Log.Error($"Exception thrown evaluating where-clause 'IfcAirTerminalBoxType.CorrectPredefinedType' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
