		...
		public bool ValidateClause(Where.IfcSpace clause) {
			...
			if (clause == Where.IfcSpace.CorrectPredefinedType) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.ProductExtension.IfcSpace");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcSpace.CorrectPredefinedType' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
			if (clause == Where.IfcSpace.CorrectTypeAssigned) {
				try {
					retVal = (SIZEOF(IsTypedBy) == 0) || (TYPEOF(this/* as IfcObject*/.IsTypedBy.ItemAt(0).RelatingType).Contains("IFC4.IFCSPACETYPE"));
				} catch (Exception ex) {
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.ProductExtension.IfcSpace");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcSpace.CorrectTypeAssigned' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
