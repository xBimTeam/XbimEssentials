...
namespace Xbim.Ifc4.HvacDomain
{
	...
	{
		private static readonly ILog Log = LogManager.GetLogger("Xbim.Ifc4.HvacDomain.IfcVibrationIsolator");
		...
		public bool ValidateClause(Where.IfcVibrationIsolator clause) {
			...
			if (clause == Where.IfcVibrationIsolator.CorrectPredefinedType) {
				try {
					...
					Log.Error($"Exception thrown evaluating where-clause 'IfcVibrationIsolator.CorrectPredefinedType' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
			if (clause == Where.IfcVibrationIsolator.CorrectTypeAssigned) {
				try {
					retVal = (SIZEOF(IsTypedBy) == 0) || (TYPEOF(this/* as IfcObject*/.IsTypedBy.ToArray()[0].RelatingType).Contains("IFC4.IFCVIBRATIONISOLATORTYPE"));
				} catch (Exception ex) {
					Log.Error($"Exception thrown evaluating where-clause 'IfcVibrationIsolator.CorrectTypeAssigned' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
