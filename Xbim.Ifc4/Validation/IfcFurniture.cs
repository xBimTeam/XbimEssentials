...
namespace Xbim.Ifc4.SharedFacilitiesElements
{
	...
	{
		private static readonly ILog Log = LogManager.GetLogger("Xbim.Ifc4.SharedFacilitiesElements.IfcFurniture");
		...
		public bool ValidateClause(Where.IfcFurniture clause) {
			...
			if (clause == Where.IfcFurniture.CorrectPredefinedType) {
				try {
					...
					Log.Error($"Exception thrown evaluating where-clause 'IfcFurniture.CorrectPredefinedType' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
			if (clause == Where.IfcFurniture.CorrectTypeAssigned) {
				try {
					retVal = (SIZEOF(IsTypedBy) == 0) || (TYPEOF(this/* as IfcObject*/.IsTypedBy.ToArray()[0].RelatingType).Contains("IFC4.IFCFURNITURETYPE"));
				} catch (Exception ex) {
					Log.Error($"Exception thrown evaluating where-clause 'IfcFurniture.CorrectTypeAssigned' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
