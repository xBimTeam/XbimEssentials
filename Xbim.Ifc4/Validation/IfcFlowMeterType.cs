		...
		public bool ValidateClause(Where.IfcFlowMeterType clause) {
			...
			if (clause == Where.IfcFlowMeterType.CorrectPredefinedType) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.HvacDomain.IfcFlowMeterType");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcFlowMeterType.CorrectPredefinedType' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
