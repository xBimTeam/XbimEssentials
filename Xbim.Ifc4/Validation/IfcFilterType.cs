		...
		public bool ValidateClause(Where.IfcFilterType clause) {
			...
			if (clause == Where.IfcFilterType.CorrectPredefinedType) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.HvacDomain.IfcFilterType");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcFilterType.CorrectPredefinedType' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
