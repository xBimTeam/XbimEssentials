...
namespace Xbim.Ifc4.HvacDomain
{
	...
	{
		private static readonly ILog Log = LogManager.GetLogger("Xbim.Ifc4.HvacDomain.IfcCooledBeam");
		...
		public bool ValidateClause(Where.IfcCooledBeam clause) {
			...
			if (clause == Where.IfcCooledBeam.CorrectPredefinedType) {
				try {
					...
					Log.Error($"Exception thrown evaluating where-clause 'IfcCooledBeam.CorrectPredefinedType' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
			if (clause == Where.IfcCooledBeam.CorrectTypeAssigned) {
				try {
					retVal = (SIZEOF(IsTypedBy) == 0) || (TYPEOF(this/* as IfcObject*/.IsTypedBy.ToArray()[0].RelatingType).Contains("IFC4.IFCCOOLEDBEAMTYPE"));
				} catch (Exception ex) {
					Log.Error($"Exception thrown evaluating where-clause 'IfcCooledBeam.CorrectTypeAssigned' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
