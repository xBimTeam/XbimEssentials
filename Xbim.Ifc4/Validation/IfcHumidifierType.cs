...
namespace Xbim.Ifc4.HvacDomain
{
	...
	{
		private static readonly ILog Log = LogManager.GetLogger("Xbim.Ifc4.HvacDomain.IfcHumidifierType");
		...
		public bool ValidateClause(Where.IfcHumidifierType clause) {
			...
			if (clause == Where.IfcHumidifierType.CorrectPredefinedType) {
				try {
					...
					Log.Error($"Exception thrown evaluating where-clause 'IfcHumidifierType.CorrectPredefinedType' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
