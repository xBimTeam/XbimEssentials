		...
		public bool ValidateClause(Where.IfcLaborResource clause) {
			...
			if (clause == Where.IfcLaborResource.CorrectPredefinedType) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.ConstructionMgmtDomain.IfcLaborResource");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcLaborResource.CorrectPredefinedType' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
