...
namespace Xbim.Ifc4.SharedBldgElements
{
	...
	{
		private static readonly ILog Log = LogManager.GetLogger("Xbim.Ifc4.SharedBldgElements.IfcBeamType");
		...
		public bool ValidateClause(Where.IfcBeamType clause) {
			...
			if (clause == Where.IfcBeamType.CorrectPredefinedType) {
				try {
					...
					Log.Error($"Exception thrown evaluating where-clause 'IfcBeamType.CorrectPredefinedType' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
