		...
		public bool ValidateClause(Where.IfcLampType clause) {
			...
			if (clause == Where.IfcLampType.CorrectPredefinedType) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.ElectricalDomain.IfcLampType");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcLampType.CorrectPredefinedType' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
