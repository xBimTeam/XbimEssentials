...
namespace Xbim.Ifc4.ProductExtension
{
	...
	{
		private static readonly ILog Log = LogManager.GetLogger("Xbim.Ifc4.ProductExtension.IfcTransportElement");
		...
		public bool ValidateClause(Where.IfcTransportElement clause) {
			...
			if (clause == Where.IfcTransportElement.CorrectPredefinedType) {
				try {
					...
					Log.Error($"Exception thrown evaluating where-clause 'IfcTransportElement.CorrectPredefinedType' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
			if (clause == Where.IfcTransportElement.CorrectTypeAssigned) {
				try {
					retVal = (SIZEOF(IsTypedBy) == 0) || (TYPEOF(this/* as IfcObject*/.IsTypedBy.ToArray()[0].RelatingType).Contains("IFC4.IFCTRANSPORTELEMENTTYPE"));
				} catch (Exception ex) {
					Log.Error($"Exception thrown evaluating where-clause 'IfcTransportElement.CorrectTypeAssigned' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
