		...
		public bool ValidateClause(Where.IfcEvaporativeCoolerType clause) {
			...
			if (clause == Where.IfcEvaporativeCoolerType.CorrectPredefinedType) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.HvacDomain.IfcEvaporativeCoolerType");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcEvaporativeCoolerType.CorrectPredefinedType' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
