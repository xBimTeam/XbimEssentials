		...
		public bool ValidateClause(Where.IfcBuildingElementPartType clause) {
			...
			if (clause == Where.IfcBuildingElementPartType.CorrectPredefinedType) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.SharedComponentElements.IfcBuildingElementPartType");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcBuildingElementPartType.CorrectPredefinedType' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
