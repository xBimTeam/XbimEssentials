...
namespace Xbim.Ifc4.HvacDomain
{
	...
	{
		private static readonly ILog Log = LogManager.GetLogger("Xbim.Ifc4.HvacDomain.IfcDuctSegmentType");
		...
		public bool ValidateClause(Where.IfcDuctSegmentType clause) {
			...
			if (clause == Where.IfcDuctSegmentType.CorrectPredefinedType) {
				try {
					...
					Log.Error($"Exception thrown evaluating where-clause 'IfcDuctSegmentType.CorrectPredefinedType' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
