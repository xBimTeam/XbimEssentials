		...
		public bool ValidateClause(Where.IfcSpaceHeater clause) {
			...
			if (clause == Where.IfcSpaceHeater.CorrectPredefinedType) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.HvacDomain.IfcSpaceHeater");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcSpaceHeater.CorrectPredefinedType' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
			if (clause == Where.IfcSpaceHeater.CorrectTypeAssigned) {
				try {
					retVal = (SIZEOF(IsTypedBy) == 0) || (TYPEOF(this/* as IfcObject*/.IsTypedBy.ItemAt(0).RelatingType).Contains("IFC4.IFCSPACEHEATERTYPE"));
				} catch (Exception ex) {
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.HvacDomain.IfcSpaceHeater");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcSpaceHeater.CorrectTypeAssigned' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
