		...
		public bool ValidateClause(Where.IfcCableCarrierFitting clause) {
			...
			if (clause == Where.IfcCableCarrierFitting.CorrectPredefinedType) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.ElectricalDomain.IfcCableCarrierFitting");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcCableCarrierFitting.CorrectPredefinedType' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
			if (clause == Where.IfcCableCarrierFitting.CorrectTypeAssigned) {
				try {
					retVal = (SIZEOF(IsTypedBy) == 0) || (TYPEOF(this/* as IfcObject*/.IsTypedBy.ItemAt(0).RelatingType).Contains("IFC4.IFCCABLECARRIERFITTINGTYPE"));
				} catch (Exception ex) {
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.ElectricalDomain.IfcCableCarrierFitting");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcCableCarrierFitting.CorrectTypeAssigned' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
