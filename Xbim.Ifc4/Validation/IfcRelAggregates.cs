		...
		public bool ValidateClause(Where.IfcRelAggregates clause) {
			...
			if (clause == Where.IfcRelAggregates.NoSelfReference) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.Kernel.IfcRelAggregates");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcRelAggregates.NoSelfReference' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
