...
namespace Xbim.Ifc4.BuildingControlsDomain
{
	...
	{
		private static readonly ILog Log = LogManager.GetLogger("Xbim.Ifc4.BuildingControlsDomain.IfcUnitaryControlElement");
		...
		public bool ValidateClause(Where.IfcUnitaryControlElement clause) {
			...
			if (clause == Where.IfcUnitaryControlElement.CorrectPredefinedType) {
				try {
					...
					Log.Error($"Exception thrown evaluating where-clause 'IfcUnitaryControlElement.CorrectPredefinedType' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
			if (clause == Where.IfcUnitaryControlElement.CorrectTypeAssigned) {
				try {
					retVal = (SIZEOF(IsTypedBy) == 0) || (TYPEOF(this/* as IfcObject*/.IsTypedBy.ToArray()[0].RelatingType).Contains("IFC4.IFCUNITARYCONTROLELEMENTTYPE"));
				} catch (Exception ex) {
					Log.Error($"Exception thrown evaluating where-clause 'IfcUnitaryControlElement.CorrectTypeAssigned' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
