		...
		public bool ValidateClause(Where.IfcCableCarrierSegment clause) {
			...
			if (clause == Where.IfcCableCarrierSegment.CorrectPredefinedType) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.ElectricalDomain.IfcCableCarrierSegment");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcCableCarrierSegment.CorrectPredefinedType' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
			if (clause == Where.IfcCableCarrierSegment.CorrectTypeAssigned) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.ElectricalDomain.IfcCableCarrierSegment");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcCableCarrierSegment.CorrectTypeAssigned' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
