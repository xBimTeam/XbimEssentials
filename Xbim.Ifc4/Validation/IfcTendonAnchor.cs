		...
		public bool ValidateClause(Where.IfcTendonAnchor clause) {
			...
			if (clause == Where.IfcTendonAnchor.CorrectPredefinedType) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.StructuralElementsDomain.IfcTendonAnchor");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcTendonAnchor.CorrectPredefinedType' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
			if (clause == Where.IfcTendonAnchor.CorrectTypeAssigned) {
				try {
					retVal = (SIZEOF(IsTypedBy) == 0) || (TYPEOF(this/* as IfcObject*/.IsTypedBy.ItemAt(0).RelatingType).Contains("IFC4.IFCTENDONANCHORTYPE"));
				} catch (Exception ex) {
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.StructuralElementsDomain.IfcTendonAnchor");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcTendonAnchor.CorrectTypeAssigned' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
