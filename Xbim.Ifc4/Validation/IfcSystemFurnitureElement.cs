		...
		public bool ValidateClause(Where.IfcSystemFurnitureElement clause) {
			...
			if (clause == Where.IfcSystemFurnitureElement.CorrectPredefinedType) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.SharedFacilitiesElements.IfcSystemFurnitureElement");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcSystemFurnitureElement.CorrectPredefinedType' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
			if (clause == Where.IfcSystemFurnitureElement.CorrectTypeAssigned) {
				try {
					retVal = (SIZEOF(IsTypedBy) == 0) || (TYPEOF(this/* as IfcObject*/.IsTypedBy.ItemAt(0).RelatingType).Contains("IFC4.IFCSYSTEMFURNITUREELEMENTTYPE"));
				} catch (Exception ex) {
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.SharedFacilitiesElements.IfcSystemFurnitureElement");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcSystemFurnitureElement.CorrectTypeAssigned' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
