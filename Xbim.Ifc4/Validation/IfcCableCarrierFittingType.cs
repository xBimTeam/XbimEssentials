...
namespace Xbim.Ifc4.ElectricalDomain
{
	...
	{
		private static readonly ILog Log = LogManager.GetLogger("Xbim.Ifc4.ElectricalDomain.IfcCableCarrierFittingType");
		...
		public bool ValidateClause(Where.IfcCableCarrierFittingType clause) {
			...
			if (clause == Where.IfcCableCarrierFittingType.CorrectPredefinedType) {
				try {
					...
					Log.Error($"Exception thrown evaluating where-clause 'IfcCableCarrierFittingType.CorrectPredefinedType' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
