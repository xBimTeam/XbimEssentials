		...
		public bool ValidateClause(Where.IfcTubeBundle clause) {
			...
			if (clause == Where.IfcTubeBundle.CorrectPredefinedType) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.HvacDomain.IfcTubeBundle");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcTubeBundle.CorrectPredefinedType' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
			if (clause == Where.IfcTubeBundle.CorrectTypeAssigned) {
				try {
					retVal = (SIZEOF(IsTypedBy) == 0) || (TYPEOF(this/* as IfcObject*/.IsTypedBy.ItemAt(0).RelatingType).Contains("IFC4.IFCTUBEBUNDLETYPE"));
				} catch (Exception ex) {
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.HvacDomain.IfcTubeBundle");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcTubeBundle.CorrectTypeAssigned' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
