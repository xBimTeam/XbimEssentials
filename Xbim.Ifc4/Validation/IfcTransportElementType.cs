...
namespace Xbim.Ifc4.ProductExtension
{
	...
	{
		private static readonly ILog Log = LogManager.GetLogger("Xbim.Ifc4.ProductExtension.IfcTransportElementType");
		...
		public bool ValidateClause(Where.IfcTransportElementType clause) {
			...
			if (clause == Where.IfcTransportElementType.CorrectPredefinedType) {
				try {
					...
					Log.Error($"Exception thrown evaluating where-clause 'IfcTransportElementType.CorrectPredefinedType' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
