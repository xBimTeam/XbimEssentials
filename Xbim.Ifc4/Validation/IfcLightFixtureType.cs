...
namespace Xbim.Ifc4.ElectricalDomain
{
	...
	{
		private static readonly ILog Log = LogManager.GetLogger("Xbim.Ifc4.ElectricalDomain.IfcLightFixtureType");
		...
		public bool ValidateClause(Where.IfcLightFixtureType clause) {
			...
			if (clause == Where.IfcLightFixtureType.CorrectPredefinedType) {
				try {
					...
					Log.Error($"Exception thrown evaluating where-clause 'IfcLightFixtureType.CorrectPredefinedType' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
