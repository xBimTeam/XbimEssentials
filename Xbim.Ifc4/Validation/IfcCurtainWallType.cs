		...
		public bool ValidateClause(Where.IfcCurtainWallType clause) {
			...
			if (clause == Where.IfcCurtainWallType.CorrectPredefinedType) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.SharedBldgElements.IfcCurtainWallType");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcCurtainWallType.CorrectPredefinedType' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
