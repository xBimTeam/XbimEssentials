...
namespace Xbim.Ifc4.StructuralAnalysisDomain
{
	...
	{
		private static readonly ILog Log = LogManager.GetLogger("Xbim.Ifc4.StructuralAnalysisDomain.IfcStructuralLoadGroup");
		...
		public bool ValidateClause(Where.IfcStructuralLoadGroup clause) {
			...
			if (clause == Where.IfcStructuralLoadGroup.HasObjectType) {
				try {
					...
					Log.Error($"Exception thrown evaluating where-clause 'IfcStructuralLoadGroup.HasObjectType' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
