...
namespace Xbim.Ifc4.ElectricalDomain
{
	...
	{
		private static readonly ILog Log = LogManager.GetLogger("Xbim.Ifc4.ElectricalDomain.IfcTransformer");
		...
		public bool ValidateClause(Where.IfcTransformer clause) {
			...
			if (clause == Where.IfcTransformer.CorrectPredefinedType) {
				try {
					...
					Log.Error($"Exception thrown evaluating where-clause 'IfcTransformer.CorrectPredefinedType' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
			if (clause == Where.IfcTransformer.CorrectTypeAssigned) {
				try {
					retVal = (SIZEOF(IsTypedBy) == 0) || (TYPEOF(this/* as IfcObject*/.IsTypedBy.ToArray()[0].RelatingType).Contains("IFC4.IFCTRANFORMERTYPE"));
				} catch (Exception ex) {
					Log.Error($"Exception thrown evaluating where-clause 'IfcTransformer.CorrectTypeAssigned' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
