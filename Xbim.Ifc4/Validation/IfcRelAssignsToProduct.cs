...
namespace Xbim.Ifc4.Kernel
{
	...
	{
		private static readonly ILog Log = LogManager.GetLogger("Xbim.Ifc4.Kernel.IfcRelAssignsToProduct");
		...
		public bool ValidateClause(Where.IfcRelAssignsToProduct clause) {
			...
			if (clause == Where.IfcRelAssignsToProduct.NoSelfReference) {
				try {
					...
					Log.Error($"Exception thrown evaluating where-clause 'IfcRelAssignsToProduct.NoSelfReference' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
