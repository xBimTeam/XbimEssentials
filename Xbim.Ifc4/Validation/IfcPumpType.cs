...
namespace Xbim.Ifc4.HvacDomain
{
	public partial class IfcPumpType : IExpressValidatable
	{
		...
		public bool ValidateClause(Where.IfcPumpType clause) {
			var retVal = false;
			if (clause == Where.IfcPumpType.CorrectPredefinedType) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.HvacDomain.IfcPumpType");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcPumpType.CorrectPredefinedType' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
		...
		public override IEnumerable<ValidationResult> Validate()
		{
			...
			if (!ValidateClause(Where.IfcPumpType.CorrectPredefinedType))
...
namespace Xbim.Ifc4.Where
{
	...
	{
		public static readonly IfcPumpType CorrectPredefinedType = new IfcPumpType();
