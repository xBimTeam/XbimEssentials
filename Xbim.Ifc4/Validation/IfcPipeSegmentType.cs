...
namespace Xbim.Ifc4.HvacDomain
{
	...
	{
		private static readonly ILog Log = LogManager.GetLogger("Xbim.Ifc4.HvacDomain.IfcPipeSegmentType");
		...
		public bool ValidateClause(Where.IfcPipeSegmentType clause) {
			...
			if (clause == Where.IfcPipeSegmentType.CorrectPredefinedType) {
				try {
					...
					Log.Error($"Exception thrown evaluating where-clause 'IfcPipeSegmentType.CorrectPredefinedType' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
