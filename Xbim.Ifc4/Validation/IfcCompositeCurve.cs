		...
		public bool ValidateClause(Where.IfcCompositeCurve clause) {
			...
			if (clause == Where.IfcCompositeCurve.CurveContinuous) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.GeometryResource.IfcCompositeCurve");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcCompositeCurve.CurveContinuous' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
			if (clause == Where.IfcCompositeCurve.SameDim) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.GeometryResource.IfcCompositeCurve");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcCompositeCurve.SameDim' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
