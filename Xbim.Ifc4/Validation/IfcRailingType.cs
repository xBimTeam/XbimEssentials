...
namespace Xbim.Ifc4.SharedBldgElements
{
	...
	{
		private static readonly ILog Log = LogManager.GetLogger("Xbim.Ifc4.SharedBldgElements.IfcRailingType");
		...
		public bool ValidateClause(Where.IfcRailingType clause) {
			...
			if (clause == Where.IfcRailingType.CorrectPredefinedType) {
				try {
					...
					Log.Error($"Exception thrown evaluating where-clause 'IfcRailingType.CorrectPredefinedType' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
