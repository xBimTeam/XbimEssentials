		...
		public bool ValidateClause(Where.IfcTendon clause) {
			...
			if (clause == Where.IfcTendon.CorrectPredefinedType) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.StructuralElementsDomain.IfcTendon");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcTendon.CorrectPredefinedType' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
			if (clause == Where.IfcTendon.CorrectTypeAssigned) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.StructuralElementsDomain.IfcTendon");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcTendon.CorrectTypeAssigned' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
