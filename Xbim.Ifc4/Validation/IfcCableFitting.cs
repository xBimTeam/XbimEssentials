		...
		public bool ValidateClause(Where.IfcCableFitting clause) {
			...
			if (clause == Where.IfcCableFitting.CorrectPredefinedType) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.ElectricalDomain.IfcCableFitting");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcCableFitting.CorrectPredefinedType' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
			if (clause == Where.IfcCableFitting.CorrectTypeAssigned) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.ElectricalDomain.IfcCableFitting");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcCableFitting.CorrectTypeAssigned' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
