...
namespace Xbim.Ifc4.ElectricalDomain
{
	...
	{
		private static readonly ILog Log = LogManager.GetLogger("Xbim.Ifc4.ElectricalDomain.IfcTransformerType");
		...
		public bool ValidateClause(Where.IfcTransformerType clause) {
			...
			if (clause == Where.IfcTransformerType.CorrectPredefinedType) {
				try {
					...
					Log.Error($"Exception thrown evaluating where-clause 'IfcTransformerType.CorrectPredefinedType' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
