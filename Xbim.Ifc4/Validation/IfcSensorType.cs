		...
		public bool ValidateClause(Where.IfcSensorType clause) {
			...
			if (clause == Where.IfcSensorType.CorrectPredefinedType) {
				try {
					...
					ILog Log = LogManager.GetLogger("Xbim.Ifc4.BuildingControlsDomain.IfcSensorType");
					Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcSensorType.CorrectPredefinedType' for #{0}.",EntityLabel), ex);
				}
				return retVal;
			}
