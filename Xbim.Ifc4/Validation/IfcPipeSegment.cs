...
namespace Xbim.Ifc4.HvacDomain
{
	...
	{
		private static readonly ILog Log = LogManager.GetLogger("Xbim.Ifc4.HvacDomain.IfcPipeSegment");
		...
		public bool ValidateClause(Where.IfcPipeSegment clause) {
			...
			if (clause == Where.IfcPipeSegment.CorrectPredefinedType) {
				try {
					...
					Log.Error($"Exception thrown evaluating where-clause 'IfcPipeSegment.CorrectPredefinedType' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
			if (clause == Where.IfcPipeSegment.CorrectTypeAssigned) {
				try {
					retVal = (SIZEOF(IsTypedBy) == 0) || (TYPEOF(this/* as IfcObject*/.IsTypedBy.ToArray()[0].RelatingType).Contains("IFC4.IFCPIPESEGMENTTYPE"));
				} catch (Exception ex) {
					Log.Error($"Exception thrown evaluating where-clause 'IfcPipeSegment.CorrectTypeAssigned' for #{EntityLabel}.", ex);
				}
				return retVal;
			}
