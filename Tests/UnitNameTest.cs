...
namespace Xbim.Essentials.Tests
{
    ...
    public class UnitNameTest
    {
        ...
        [TestMethod]
        public void NamedUnitTest4x3()
        {
            using (var model = new MemoryModel(new Ifc4x3.EntityFactoryIfc4x3Add2()))
            {
                Ifc4x3  .MeasureResource.IfcSIUnit unit;
                using (var txn = model.BeginTransaction(""))
                {
                    unit = model.Instances.New<Ifc4x3.MeasureResource.IfcSIUnit>(u =>
                    {
                        u.Name = Ifc4x3.MeasureResource.IfcSIUnitName.METRE;
                        u.Prefix = Ifc4x3.MeasureResource.IfcSIPrefix.MILLI;
                    });
                    txn.Commit();
                }
                Assert.IsNotNull(unit);
